...
Capabilities
Fully external — runs as a separate process; does not modify game memory or code
...
Note on actor detection:
The tool reads GameState -> PlayerArray to enumerate other players. To see bounding boxes, load into a match, lobby, or any environment where remote actors exist. If no other players are present, enable Show Local Player to test the projection and rendering pipeline with your own character.
